#!/usr/bin/env bash
#
# install_akms.sh - tcp-brutal akms module install script
# Try `install_akms.sh --help` for usage.
#
...
check_environment_akms() {
  if has_command akms; then
    return
  fi

  install_software akms
}
...
check_linux_headers() {
  ...
  if is_linux_headers_installed; then
    ...
    if ! install_linux_headers; then
      ...
      warning "The AKMS kernel module will not be compiled."
    fi
  fi
}
...
show_usage_and_exit() {
  echo
  echo -e "\t${tbold}$(script_name)${treset} - tcp-brutal akms install script"
  echo
  echo -e "Usage:"
  echo
  ...
  echo -e "\t-l, --local <file>\tInstall specified AKMS tarball instead of download it."
...
akms_get_installed_versions() {
  local _module="$1"

  local _akms_moddir="/var/lib/akms/$_module"

  if [[ ! -d "$_akms_moddir" ]]; then
    return
  fi

  for file in $(command ls "$_akms_moddir/"); do
    if [[ -L "$_akms_moddir/$file" ]]; then
...
akms_remove_modules() {
  ...
  local _versions_to_remove=($(akms_get_installed_versions "$_module"))
  ...
  for version in "${_versions_to_remove[@]}"; do
    local _akms_version="${version#v}"

    echo -n "Removing AKMS module $_module/$_akms_version ... "
    if akms remove "$_module/$_akms_version" --all > /dev/null; then
      echo "ok"
    else
      # suppress akms remove failed, shall not to be a problem
      continue
    fi
    echo -n "Cleaning AKMS module source /usr/src/$_module-$_akms_version ... "
    if rm -rf "/usr/src/$_module-$_akms_version"; then
      echo "ok"
    else
      # also suppress this
      continue
...
akms_ldtarball() {
  ...
  source "$_extractdir/akms_source_tree/akms.conf"

  if [[ -z "$PACKAGE_NAME" || -z "$PACKAGE_VERSION" ]]; then
    error "Malformed AKMS tarball, PACKAGE_NAME or PACKAGE_VERSION is missing."
    exit 22
  fi

  rm -rf "/usr/src/$PACKAGE_NAME-$PACKAGE_VERSION"
  mkdir -p "/usr/src/$PACKAGE_NAME-$PACKAGE_VERSION"
  cp -a "$_extractdir/akms_source_tree/." "/usr/src/$PACKAGE_NAME-$PACKAGE_VERSION/"
  rm -rf "$_extractdir"

  akms add "$PACKAGE_NAME/$PACKAGE_VERSION"
}

akms_install_tarball() {
  local _tarball="$1"

  echo "Installing AKMS module from tarball file $_tarball ... "
  if ! akms_ldtarball "$_tarball"; then
    error "Failed to install AKMS tarball, please check above output or try to uninstall first."
    return 1
  fi
}
...
download_akms_tarball() {
  local _version="$1"
  local _destination="$2"

  local _download_url="$REPO_URL/releases/download/$_version/tcp-brutal.akms.tar.gz"
  echo "Downloading AKMS tarball: $_download_url ..."
...
perform_install() {
    ...
    case "$1" in
      ...
      '--local' | '-l')
        ...
        if [[ -z "$_local_file" ]]; then
          show_argument_error_and_exit "Please specify the local akms.tar file to install for option '-l' or '--local'."
        fi
        _install_needed="1"
  ...
  akms_remove_modules "$AKMS_MODULE_NAME" "1"

  echo -n "Checking installed version ... "
  local _installed_version="$(akms_get_installed_versions "$AKMS_MODULE_NAME" | head -1)"
  ...
  if [[ -z "$_local_file" && -n "$_version" ]]; then
    ...
    if [[ -n "$_install_needed" ]]; then
      ...
      download_akms_tarball "$_version" "$_download_destination"
      _local_file="$_download_destination"
    fi
  fi

  if [[ -n "$_install_needed" ]]; then
    # remove all installed version as AKMS not allowed to overwrite a installed module
    akms_remove_modules "$AKMS_MODULE_NAME" ""
    akms_install_tarball "$_local_file"
  fi
  ...
  echo "Rebuilding AKMS modules as needed ... "
  if ! akms autoinstall; then
    warning "Error occurred in 'akms autoinstall', please check above output."
  fi
...
perform_check() {
  ...
  echo -n "Checking installed version ... "
  local _installed_versions=($(akms_get_installed_versions "$AKMS_MODULE_NAME"))
